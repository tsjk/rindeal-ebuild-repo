...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-0.19.7-disable-libintl.patch #564168
	eapply_user
...
src_configure() {
	local myconf=(
		...
		$(use_enable java)
		$(use_enable java-native native-java)
...
src_install() {
	...
	use nls || erm -r "${ED}"/usr/share/locale
	use static-libs || prune_libtool_files --all

	erm "${ED}"/usr/share/locale/locale.alias "${ED}"/usr/lib/charset.alias

	if use java ; then
		java-pkg_dojar "${ED}"/usr/share/${PN}/*.jar
		erm "${ED}"/usr/share/${PN}/*.jar
		erm "${ED}"/usr/share/${PN}/*.class
		if use doc ; then
			java-pkg_dojavadoc "${ED}"/usr/share/doc/${PF}/javadoc2
			erm -r "${ED}"/usr/share/doc/${PF}/javadoc2
		fi
	fi
	...
	if use doc ; then
		...
		erm -r "${ED}"/usr/share/doc/${PF}/{csharpdoc,examples,javadoc2,javadoc1}
	fi
	erm "${ED}"/usr/share/doc/${PF}/*.html
}
